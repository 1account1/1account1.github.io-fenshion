...
fetch("https://opensheet.elk.sh/1SKotYYErxLQCxm9Y3VumQw07fl7DFklMaRM7bSVAcVg/Sheet7")
  .then(res => res.json())
  .then(data => {
        data.forEach(item => {
        console.log(item);
...
async function mechu() {
    ...
    if (value.replace(/[^a-zA-Z0-9]/g, "").includes("경민") == false){
        ...
        const res = await fetch(
            "https://sheetdb.io/api/v1/pkm2qg1f67mhp?sheet=Sheet7", 
            {
                method: "POST",
                headers: {
